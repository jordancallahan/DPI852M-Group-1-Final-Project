...
confidence_data = pd.DataFrame({
    "Institution": ["Scientific community", "The military", "The Supreme Court", "Organized religion",
                    "Banks and financial institutions", "The press", "Congress"],
    "Percentage": [44, 33, 26, 20, 14, 9, 8],
})

# Bar chart for Public Confidence in Leaders of Institutions (2018)
confidence_chart = go.Figure(
    go.Bar(x=confidence_data["Institution"], y=confidence_data["Percentage"], marker_color='mediumseagreen')
)
confidence_chart.update_layout(title="Percentage of People Expressing a Great Deal of Confidence in Leaders of Institutions (2018)", xaxis_title="Institution", yaxis_title="Percentage", showlegend=False)
...
funding_chart.add_trace(go.Scatter(x=funding_data["Year"], y=funding_data["Too little"], mode='lines+markers', name="Too little", marker_color='royalblue'))
funding_chart.add_trace(go.Scatter(x=funding_data["Year"], y=funding_data["About right"], mode='lines+markers', name="About right", marker_color='firebrick'))
...
interest_chart = go.Figure(go.Scatter(x=interest_data["Year"], y=interest_data["Percentage"], mode='lines+markers', marker_color='blue'))
...
understanding_chart = go.Figure(go.Scatter(x=understanding_data["Year"], y=understanding_data["Percentage"], mode='lines+markers', marker_color='darkorange'))
...
environment_chart = go.Figure(
    go.Bar(x=environment_data["Opinion"], y=environment_data["Percentage"], marker_color='dodgerblue')
)
environment_chart.update_layout(title="Opinion on Science's Effect on the Environment (2018)", xaxis_title="Opinion", yaxis_title="Percentage", showlegend=False)
...
funding_field_chart = go.Figure(
    go.Bar(x=funding_field_data["Field"], y=funding_field_data["Percentage"], marker_color='teal')
)
funding_field_chart.update_layout(title="Importance of Government Funding by Field (2018)", xaxis_title="Field", yaxis_title="Percentage", showlegend=False)
...
knowledge_data = pd.DataFrame({
    "Question": ["Antibiotics", "Evolution", "Astronomy", "Geology", "Radiation", "Vaccines", "Genetics"],
